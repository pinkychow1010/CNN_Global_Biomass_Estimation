...
#############################################
### Baseline CNN with Monte Carlo Dropout ###
#############################################
...
class BaseNeuralNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        # Applying dropout for Monte Carlo sampling
        x10_dropout = self.dropout(x10)

        out = self.output_conv(x10_dropout)

        if self.output_variance:
            # Make sure that the variance is positive
            var = torch.abs(self.variance(x10_dropout))
            out = torch.cat([out, var], dim=1)

        return out

    def predict_step(self, batch, batch_idx, dataloader_idx=None):
        x, _ = batch  # Assuming your dataloader returns (input, target) pairs
        outputs = []
        ...
        return {"mean_output": mean_output, "std_output": std_output}
